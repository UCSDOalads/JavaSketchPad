...
public class SimplePoint extends PaintComponent {
	...
	public SimplePoint(int x, int y) {
		this(x, y, Defaults.sharedDefaults().defaultSimplePointSize(), Defaults
				.sharedDefaults().defaultSimplePointColor(), Defaults
				.sharedDefaults().defaultSimplePointSelectedColor());
	}

	public SimplePoint(int x, int y, int radius, Color color,
			Color selectedColor) {
		super(x, y);
		this.radius = radius;
		this.color = color;
	...
	protected void paintNotSelected(Graphics g) {
		g.setColor(color);
		g.fillOval(this.getX() - this.radius / 2,
				this.getY() - this.radius / 2, radius, radius);
	}
	...
	public boolean contains(int x, int y) {
		return new Rectangle(this.getX() - this.radius / 2, this.getY()
				- this.radius / 2, radius, radius).contains(x, y);
	}
	...
	public void saveToElement(Element rootElement, Document doc) {
		...
		XMLEncodingUtilForPaintComponents.attachRGB(selectedColor,
				selectedColorElem, doc);
	...
	public SimplePoint(Element rootElement, PaintPanel panel) {
		...
		Element main = (Element) rootElement
				.getElementsByTagName("simplepoint").item(0);
		Element radiusElem = (Element) main.getElementsByTagName("radius")
				.item(0);
		Element colorElem = (Element) main.getElementsByTagName("color")
				.item(0);
		Element selectedColorElem = (Element) main.getElementsByTagName(
				"selectedcolor").item(0);

		this.radius = Integer.parseInt(radiusElem.getTextContent());
		this.color = XMLEncodingUtilForPaintComponents.getRGB(colorElem);
		this.selectedColor = XMLEncodingUtilForPaintComponents
				.getRGB(selectedColorElem);
